...
export class baseGameMode {
  ...
  setBlockTouch(player: Player) {
    ...
    if (ball.yspeed) ball.yspeed /= 2.5;
    if (ball.xspeed) ball.xspeed /= 2;
    ball.cGroup = 0;
    ball.cMask = 0;
  ...
  @Event onPlayerBallKick(player: Player) {
    ...
    if (lastTouchers.length === 0) return;
    if (
      lastTouchers[1] &&
      ...
      this.countConsecutiveTouches(lastTouchers, lastTouchers[0].team) >= 4 &&
  ...
  private countConsecutiveTouches(touchers: Player[], team: number) {
    let count = 0;
    for (const toucher of touchers) {
      if (!toucher || toucher.team != team) break;
      count++;
    }
    return count;
  }
  ...
  @CustomEvent onClockTick() {
    ...
    if(this.saqueTimeout == 14) {
      this.$.send({ message: `[🤾] ${this.currentServer.name} tem 5 segundos para sacar.`, color: Colors.AquaMarine });
