...
exports.ECONOMY_CONFIG = exports.EconomyModule = void 0;
exports.getWealthTitle = getWealthTitle;
...
exports.ECONOMY_CONFIG = ECONOMY_CONFIG;
const LEGACY_GEM_TO_COINS_RATE = 1000;
function getWealthTitle(coins) {
...
function calculateOdds(redElo, blueElo) {
...
let EconomyModule = class EconomyModule {
    constructor($) {
        ...
        this.betPool = {};
        this.betsOpen = false;
        this.currentOdds = { redOdd: 2.0, blueOdd: 2.0 };
    }
    // ---- HELPERS ----
    getOrCreateEconomy(discordId) {
        return __awaiter(this, void 0, void 0, function* () {
            let eco = yield prisma_1.prisma.economy.findUnique({ where: { discordId } });
            if (!eco) {
                eco = yield prisma_1.prisma.economy.create({ data: { discordId } });
            }
            if (eco.gems > 0) {
                const gemAmount = eco.gems;
                const converted = gemAmount * LEGACY_GEM_TO_COINS_RATE;
                eco = yield prisma_1.prisma.economy.update({
                    where: { id: eco.id },
                    data: { coins: { increment: converted }, gems: 0 },
                });
                yield prisma_1.prisma.transaction.create({
                    data: {
                        economyId: eco.id,
                        type: "earn",
                        amount: converted,
    ...
    addCoins(discordId, amount, description) {
        return __awaiter(this, void 0, void 0, function* () {
            const eco = yield this.getOrCreateEconomy(discordId);
            const multiplier = eco.multiplierExp && eco.multiplierExp > new Date() ? eco.multiplier : 1.0;
            const finalAmount = Math.floor(amount * multiplier);
            const updated = yield prisma_1.prisma.economy.update({
                where: { discordId },
                data: {
    ...
    removeCoins(discordId_1, amount_1, description_1) {
        return __awaiter(this, arguments, void 0, function* (discordId, amount, description, type = "spend") {
            const eco = yield this.getOrCreateEconomy(discordId);
            if (eco.coins < amount)
                return null;
            const updated = yield prisma_1.prisma.economy.update({
    ...
    getTeamAvgElo(team) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            for (const p of players) {
                if ((_a = p.settings.account) === null || _a === void 0 ? void 0 : _a.discordId) {
                    const status = yield prisma_1.prisma.status.findFirst({ where: { discordId: p.settings.account.discordId } });
                    elos.push((status === null || status === void 0 ? void 0 : status.elo) || 1000);
                }
                else {
                    elos.push(1000);
                }
    ...
    // ---- GANHOS POR PARTIDA ----
    addMatchEarning(player, amount) {
        var _a;
        const discordId = (_a = player.settings.account) === null || _a === void 0 ? void 0 : _a.discordId;
        if (!discordId)
            return;
        if (!this.matchEarnings[discordId])
            this.matchEarnings[discordId] = 0;
    ...
    onGameStart() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (red === settings_json_1.default.mapPlayers && blue === settings_json_1.default.mapPlayers) {
                this.betsOpen = true;
                const redElo = yield this.getTeamAvgElo(1);
                const blueElo = yield this.getTeamAvgElo(2);
    ...
    onTeamVictory() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (this.$.players.teams().size < settings_json_1.default.mapPlayers * 2) {
                this.matchEarnings = {};
                this.betPool = {};
                this.betsOpen = false;
                return;
            ...
            for (const p of players) {
                const discordId = (_a = p.settings.account) === null || _a === void 0 ? void 0 : _a.discordId;
                if (!discordId)
                    continue;
                ...
                const eco = yield this.addCoins(discordId, earnings, isWinner ? "Vitória na partida" : "Derrota na partida");
            ...
            yield this.resolveBets(winnerTeam);
            this.matchEarnings = {};
            this.betsOpen = false;
        });
    }
    resolveBets(winnerTeam) {
        return __awaiter(this, void 0, void 0, function* () {
            if (Object.keys(this.betPool).length === 0)
                return;
            ...
            for (const [playerIdStr, bet] of Object.entries(this.betPool)) {
                ...
                if (bet.team === winnerTeam) {
                    ...
                    yield prisma_1.prisma.economy.update({
                    ...
                    yield prisma_1.prisma.transaction.create({
                        data: {
                            economyId: (yield this.getOrCreateEconomy(bet.discordId)).id,
                            type: "bet_win",
                            amount: winnings,
    ...
    checkBalance(command) {
        return __awaiter(this, void 0, void 0, function* () {
            if (!command.player.settings.account) {
                command.player.reply(messageFormatter_1.MessageFormatter.error("Registre-se primeiro! Use !discord"));
                return;
            }
            const discordId = command.player.settings.account.discordId;
            const eco = yield this.getOrCreateEconomy(discordId);
            const status = yield prisma_1.prisma.status.findFirst({ where: { discordId } });
            const wealth = getWealthTitle(eco.coins);
            ...
            // Header
            command.player.reply(messageFormatter_1.MessageFormatter.header(`💳 ${command.player.name}`));
            // Moedas
            ...
            if (multiplierActive) {
                command.player.reply(messageFormatter_1.MessageFormatter.success(`Multiplicador ativo: ${eco.multiplier}x!`));
            }
        });
    }
    oddsCommand(command) {
        return __awaiter(this, void 0, void 0, function* () {
            if (!this.betsOpen) {
                command.player.reply(messageFormatter_1.MessageFormatter.warning("Apostas não estão abertas no momento"));
                return;
            }
            command.player.reply(messageFormatter_1.MessageFormatter.betting(`Red ${this.currentOdds.redOdd}x • Blue ${this.currentOdds.blueOdd}x`));
        });
    }
    dailyReward(command) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (eco.lastDaily) {
                ...
                if (hoursSince > 48) {
                    yield prisma_1.prisma.economy.update({ where: { discordId }, data: { dailyStreak: 0 } });
                    eco.dailyStreak = 0;
                }
            }
            ...
            yield this.addCoins(discordId, totalCoins, `Recompensa diária (streak ${newStreak})`);
            const dailyMsg = messageFormatter_1.MessageFormatter.economy(`🎁 Diário coletado!`, `+${formatCoins(totalCoins)} moedas`);
            command.player.reply(dailyMsg);
            const streakMsg = messageFormatter_1.MessageFormatter.stat(`🔥 Streak: ${newStreak} dias`, `Bônus: +${formatCoins(streakBonus)}`);
    ...
    betCommand(command) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (!this.betsOpen) {
                command.player.reply(messageFormatter_1.MessageFormatter.error("Apostas fechadas!", "Só funciona em 3x3 nos primeiros 15s"));
                return;
            }
            if (this.betPool[command.player.id]) {
                command.player.reply(messageFormatter_1.MessageFormatter.error("Aposta duplicada!", "Você já apostou nesta partida"));
                return;
            }
            const args = command.arguments;
            if (args.length < 2) {
                command.player.reply(messageFormatter_1.MessageFormatter.warning("Uso: !apostar <valor> <red/blue>", ""));
                command.player.reply(messageFormatter_1.MessageFormatter.betting(`🔴 Red: ${this.currentOdds.redOdd}x`, `🔵 Blue: ${this.currentOdds.blueOdd}x`));
                return;
            }
            const amount = parseInt(args[0].toString());
            const teamStr = args[1].toString().toLowerCase();
            const team = teamStr === "red" || teamStr === "vermelho" || teamStr === "r" ? 1
                : teamStr === "blue" || teamStr === "azul" || teamStr === "b" ? 2
                    : null;
            if (!team) {
                command.player.reply(messageFormatter_1.MessageFormatter.error("Time inválido!", "Use: red ou blue"));
                return;
            }
            if (isNaN(amount) || amount < ECONOMY_CONFIG.bet.minBet) {
                command.player.reply(messageFormatter_1.MessageFormatter.error(`Aposta mínima: ${formatCoins(ECONOMY_CONFIG.bet.minBet)}`, ""));
                return;
            }
            if (amount > ECONOMY_CONFIG.bet.maxBet) {
                command.player.reply(messageFormatter_1.MessageFormatter.error(`Aposta máxima: ${formatCoins(ECONOMY_CONFIG.bet.maxBet)}`, ""));
                return;
            }
            const discordId = command.player.settings.account.discordId;
            const result = yield this.removeCoins(discordId, amount, `Aposta: ${amount} no ${team === 1 ? "Red" : "Blue"} (odd ${team === 1 ? this.currentOdds.redOdd : this.currentOdds.blueOdd}x)`, "bet");
            if (!result) {
                command.player.reply(messageFormatter_1.MessageFormatter.error("Saldo insuficiente!", ""));
                return;
            }
            yield prisma_1.prisma.economy.update({
                where: { discordId },
            ...
            const betAnnounce = messageFormatter_1.MessageFormatter.betting(`${command.player.name} apostou ${formatCoins(amount)}`, `${team === 1 ? "🔴 Red" : "🔵 Blue"} (${odd}x) → ${formatCoins(potentialWin)}`);
    ...
    tigrinhoCommand(command) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const args = command.arguments;
            if (args.length < 1) {
                command.player.reply(messageFormatter_1.MessageFormatter.warning("Uso: !tigrinho <valor>", ""));
                command.player.reply(messageFormatter_1.MessageFormatter.betting(`Mín: ${ECONOMY_CONFIG.tigrinho.minBet}`, `Máx: ${formatCoins(ECONOMY_CONFIG.tigrinho.maxBet)}`));
                return;
            }
            const amount = parseInt(args[0].toString());
            if (isNaN(amount) || amount < ECONOMY_CONFIG.tigrinho.minBet) {
                command.player.reply(messageFormatter_1.MessageFormatter.error(`Aposta mínima: ${ECONOMY_CONFIG.tigrinho.minBet}`, ""));
                return;
            }
            if (amount > ECONOMY_CONFIG.tigrinho.maxBet) {
                command.player.reply(messageFormatter_1.MessageFormatter.error(`Aposta máxima: ${formatCoins(ECONOMY_CONFIG.tigrinho.maxBet)}`, ""));
                return;
            }
            const discordId = command.player.settings.account.discordId;
            const eco = yield this.getOrCreateEconomy(discordId);
            ...
            if (eco.lastTigrinho) {
                ...
                if (secsSince < ECONOMY_CONFIG.tigrinho.cooldownSeconds) {
                    ...
                    command.player.reply(messageFormatter_1.MessageFormatter.warning(`Aguarde ${secsLeft}s`, ""));
                    return;
                }
            }
            const removeResult = yield this.removeCoins(discordId, amount, `Tigrinho: ${amount}`, "bet");
            if (!removeResult) {
                command.player.reply(messageFormatter_1.MessageFormatter.error("Saldo insuficiente!", ""));
                return;
            }
            yield prisma_1.prisma.economy.update({ where: { discordId }, data: { lastTigrinho: new Date() } });
            // Girar 3 slots
            const s1 = spinSymbol();
            const s2 = spinSymbol();
            const s3 = spinSymbol();
            ...
            if (s1.emoji === s2.emoji && s2.emoji === s3.emoji) {
                ...
                yield this.addCoins(discordId, winnings, `Tigrinho JACKPOT (${multiplier}x)`);
                yield prisma_1.prisma.economy.update({
                    where: { discordId },
                    data: { totalBetWon: { increment: profit }, betsWon: { increment: 1 }, totalBets: { increment: 1 } },
                });
                this.$.send(messageFormatter_1.MessageFormatter.betting(`🐯 ${command.player.name} GANHOU!`, line));
                const result = messageFormatter_1.MessageFormatter.economy(`🤑 JACKPOT ${multiplier}x!`, `+${formatCoins(winnings)} (lucro: ${formatCoins(profit)})`);
                this.$.send(result);
                if (s1.emoji === "🐯") {
                    const legendaryMsg = messageFormatter_1.MessageFormatter.success(`🐯🐯🐯 TIGRINHO TRIPLO!!!`, `${command.player.name} ganhou ${formatCoins(winnings)}`);
                    this.$.send(legendaryMsg);
                }
            }
            else if (s1.emoji === s2.emoji || s2.emoji === s3.emoji || s1.emoji === s3.emoji) {
                // 2 iguais - retorna a aposta
                yield this.addCoins(discordId, amount, `Tigrinho: 2 iguais (devolução)`);
                yield prisma_1.prisma.economy.update({
                    where: { discordId },
                    data: { totalBets: { increment: 1 } },
                });
                const almostMsg = messageFormatter_1.MessageFormatter.info(`${line}`, "Quase! Aposta devolvida");
                ...
                const lostMsg = messageFormatter_1.MessageFormatter.error(`${line}`, `-${formatCoins(amount)}`);
    ...
    transferCommand(command) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const args = command.arguments;
            if (args.length < 2) {
                command.player.reply(messageFormatter_1.MessageFormatter.warning("Uso: !transferir <valor> @jogador", ""));
                return;
            }
            const amount = parseInt(args[0].toString());
            if (isNaN(amount) || amount <= 0) {
                command.player.reply(messageFormatter_1.MessageFormatter.error("Valor inválido!", ""));
                return;
            }
            ...
            if (!targetPlayer || !targetPlayer.settings.account) {
                command.player.reply(messageFormatter_1.MessageFormatter.error("Jogador não encontrado!", "Ou não registrado"));
                return;
            }
            if (targetPlayer.id === command.player.id) {
                command.player.reply(messageFormatter_1.MessageFormatter.error("Não pode transferir para si mesmo!", ""));
                return;
            }
            ...
            const result = yield this.removeCoins(senderDiscordId, amount, `Transferência para ${targetPlayer.name} (taxa: ${tax})`, "transfer_out");
            if (!result) {
                command.player.reply(messageFormatter_1.MessageFormatter.error("Saldo insuficiente!", ""));
                return;
            }
            yield this.addCoins(receiverDiscordId, netAmount, `Transferência de ${command.player.name}`);
            const senderMsg = messageFormatter_1.MessageFormatter.economy(`💸 Transferência enviada`, `${formatCoins(amount)} → ${targetPlayer.name}\nTaxa: ${formatCoins(tax)}`);
            command.player.reply(senderMsg);
            const receiverMsg = messageFormatter_1.MessageFormatter.success(`💸 Moedas recebidas!`, `+${formatCoins(netAmount)} de ${command.player.name}`);
    ...
    rankingCommand(command) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            switch (category) {
                ...
                case "moedas":
                case "ricos":
                    yield this.showRanking(command.player, "coins", "🪙 TOP MAIS RICOS");
                    break;
                case "elo":
                case "rank":
                    yield this.showEloRanking(command.player);
                    break;
                case "cortes":
                case "corte":
                    yield this.showStatusRanking(command.player, "cortes", "⚽ TOP CORTES");
                    break;
                case "levants":
                case "levant":
                case "levantamentos":
                    yield this.showStatusRanking(command.player, "levants", "🏌️ TOP LEVANTAMENTOS");
                    break;
                case "blocks":
                case "block":
                case "bloqueios":
                    yield this.showStatusRanking(command.player, "blocks", "🛡️ TOP BLOQUEIOS");
                    break;
                case "wins":
                case "vitorias":
                    yield this.showStatusRanking(command.player, "wins", "✅ TOP VITÓRIAS");
                    break;
                case "apostas":
                case "bets":
                    yield this.showBetRanking(command.player);
                    break;
                default:
                    command.player.reply({
    ...
    showRanking(player, field, title) {
        return __awaiter(this, void 0, void 0, function* () {
            const topPlayers = yield prisma_1.prisma.economy.findMany({
                orderBy: { [field]: "desc" },
                take: 5,
            });
            player.reply(messageFormatter_1.MessageFormatter.header(title));
            for (let i = 0; i < topPlayers.length; i++) {
                ...
                const wealth = getWealthTitle(eco.coins);
                player.reply({
                    message: `${medal} ${(user === null || user === void 0 ? void 0 : user.nickname) || "???"} - ${formatCoins(eco.coins)} moedas (${wealth.title})`,
                    color: wealth.color,
                });
            }
        });
    }
    showEloRanking(player) {
        return __awaiter(this, void 0, void 0, function* () {
            const topPlayers = yield prisma_1.prisma.status.findMany({
                orderBy: { elo: "desc" },
                take: 5,
            });
            player.reply(messageFormatter_1.MessageFormatter.header("⭐ TOP ELO"));
            for (let i = 0; i < topPlayers.length; i++) {
                ...
                player.reply({
                    message: `${medal} ${(user === null || user === void 0 ? void 0 : user.nickname) || "???"} - ${st.elo} ELO (${rank})`,
                    color: 0x00BFFF,
                });
            }
        });
    }
    showStatusRanking(player, field, title) {
        return __awaiter(this, void 0, void 0, function* () {
            const topPlayers = yield prisma_1.prisma.status.findMany({
                orderBy: { [field]: "desc" },
                take: 5,
            ...
            for (let i = 0; i < topPlayers.length; i++) {
                ...
                player.reply({
                    message: `${medal} ${(user === null || user === void 0 ? void 0 : user.nickname) || "???"} - ${value}`,
                    color: 0x00BFFF,
                });
            }
        });
    }
    showBetRanking(player) {
        return __awaiter(this, void 0, void 0, function* () {
            const topPlayers = yield prisma_1.prisma.economy.findMany({
                orderBy: { totalBetWon: "desc" },
                take: 5,
            });
            player.reply(messageFormatter_1.MessageFormatter.header("🎰 TOP APOSTADORES"));
            for (let i = 0; i < topPlayers.length; i++) {
                const eco = topPlayers[i];
                const user = yield prisma_1.prisma.user.findFirst({ where: { discordId: eco.discordId } });
                const medal = i === 0 ? "🥇" : i === 1 ? "🥈" : i === 2 ? "🥉" : `#${i + 1}`;
                ...
                player.reply({
                    message: `${medal} ${(user === null || user === void 0 ? void 0 : user.nickname) || "???"} - Lucro: ${formatCoins(profit)} | ${eco.betsWon}W/${eco.betsLost}L (${wr}%)`,
                    color: profit >= 0 ? haxball_extended_room_1.Colors.GreenYellow : haxball_extended_room_1.Colors.Red,
                });
            }
        });
    }
    ...
    shopCommand(command) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (category && ECONOMY_CONFIG.shopCategories[category]) {
                yield this.showShopCategory(command.player, category);
                return;
            }
            // Mostrar menu de categorias (compacto)
            const parts = [];
            for (const [key, cat] of Object.entries(ECONOMY_CONFIG.shopCategories)) {
                const count = yield prisma_1.prisma.shopItem.count({ where: { category: key, isActive: true, currency: "coins" } });
                if (count > 0)
                    parts.push(`${cat.emoji} ${key}(${count})`);
            }
            const summary = parts.length > 0 ? parts.join(" | ") : "Loja vazia";
            command.player.reply(messageFormatter_1.MessageFormatter.info("Loja", `${summary} | !loja <categoria> | !comprar <id>`));
        });
    }
    showShopCategory(player, category) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (items.length === 0) {
                player.reply(messageFormatter_1.MessageFormatter.warning("Nenhum item nesta categoria"));
                return;
            }
            player.reply(messageFormatter_1.MessageFormatter.info(`${catInfo.emoji} ${catInfo.label}`, `!comprar <id>`));
    ...
    buyCommand(command) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const args = command.arguments;
            if (args.length === 0) {
                command.player.reply(messageFormatter_1.MessageFormatter.warning("Uso: !comprar <id>", "Use !loja"));
                return;
            }
            let quantity = 1;
            ...
            const item = yield prisma_1.prisma.shopItem.findFirst({
                where: {
                    isActive: true,
                    currency: "coins",
                    OR: [
            ...
            if (!item) {
                command.player.reply(messageFormatter_1.MessageFormatter.error("Item não encontrado!", "Use !loja"));
                return;
            }
            if (item.category !== "fila") {
                ...
                command.player.reply(messageFormatter_1.MessageFormatter.error("Quantidade inválida!", "Use !comprar furafila <quantidade>"));
                return;
            }
            if (item.stock === 0) {
                command.player.reply(messageFormatter_1.MessageFormatter.error("Item esgotado!"));
                return;
            }
            if (item.stock > 0 && quantity > item.stock) {
                command.player.reply(messageFormatter_1.MessageFormatter.error("Estoque insuficiente!", `Disponivel: ${item.stock}`));
                return;
            }
            const discordId = command.player.settings.account.discordId;
            const eco = yield this.getOrCreateEconomy(discordId);
            ...
            if (item.maxPerUser > 0) {
                const userPurchases = yield prisma_1.prisma.purchase.count({
                    where: { economy: { discordId }, itemId: item.id },
                });
                if (userPurchases >= item.maxPerUser) {
                    command.player.reply(messageFormatter_1.MessageFormatter.error("Limite de compras atingido!", `Máximo: ${item.maxPerUser}`));
                    return;
                }
            }
            // Verificar saldo
            ...
            if (eco.coins < totalPrice) {
                command.player.reply(messageFormatter_1.MessageFormatter.error(`Saldo insuficiente!`, `Preço: ${formatCoins(totalPrice)} 🪙`));
                return;
            }
            // Cobrar
            yield this.removeCoins(discordId, totalPrice, `Compra: ${item.name}`, "spend");
            // Reduzir estoque
            if (item.stock > 0) {
                yield prisma_1.prisma.shopItem.update({ where: { id: item.id }, data: { stock: { decrement: quantity } } });
            }
            ...
            yield prisma_1.prisma.purchase.create({
                data: {
                    economyId: eco.id,
                    itemId: item.id,
                    quantity,
            ...
            if (item.category === "lootbox") {
                yield this.openLootbox(command.player, item, eco.id);
                return;
            }
            // Aplicar efeito do item
            if (item.category === "fila" && quantity > 1) {
                command.player.reply(messageFormatter_1.MessageFormatter.success("🚀 Furafila adquirido!", `${quantity} usos`));
            }
            else {
                yield this.applyItemEffect(command.player, item);
            }
            const rarityInfo = ECONOMY_CONFIG.rarities[item.rarity] || ECONOMY_CONFIG.rarities.common;
            const codeBase = item.code || item.name.replace(/\s+/g, "").toLowerCase();
            const codeLabel = `id${codeBase}`;
            command.player.reply(messageFormatter_1.MessageFormatter.success("Compra realizada!", `${rarityInfo.emoji} ${codeLabel} — ${formatCoins(totalPrice)} 🪙`));
    ...
    //  📦 LOOTBOX
    // ========================
    openLootbox(player, lootboxItem, economyId) {
        return __awaiter(this, void 0, void 0, function* () {
            var _a;
            const discordId = (_a = player.settings.account) === null || _a === void 0 ? void 0 : _a.discordId;
            if (!discordId)
            ...
            const possibleItems = yield prisma_1.prisma.shopItem.findMany({
                where: {
                    rarity: wonRarity,
            ...
            if (possibleItems.length === 0) {
                ...
                yield this.addCoins(discordId, coinsReward, `Lootbox: prêmio em moedas (${rarityInfo.label})`);
                player.reply(messageFormatter_1.MessageFormatter.economy(`Lootbox: ${rarityInfo.emoji} ${rarityInfo.label}`, `+${formatCoins(coinsReward)} moedas`));
                return;
            }
            ...
            const alreadyOwns = yield prisma_1.prisma.purchase.findFirst({
            ...
            if (alreadyOwns && wonItem.category === "avatar") {
                ...
                yield this.addCoins(discordId, coinsCompensation, `Lootbox: duplicata (${wonItem.name})`);
                player.reply(messageFormatter_1.MessageFormatter.warning(`Lootbox: ${rarityInfo.emoji} ${wonItem.name}`, `Duplicata +${formatCoins(coinsCompensation)} 🪙`));
            }
            else {
                ...
                player.reply(messageFormatter_1.MessageFormatter.success(`Lootbox: ${rarityInfo.emoji} ${wonItem.name}`, "Item adicionado"));
                // Aplicar efeito se não for avatar (avatares precisam ser equipados)
                if (wonItem.category !== "avatar") {
                    yield this.applyItemEffect(player, wonItem);
                }
            }
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (args.length === 0) {
                ...
                if (purchases.length === 0) {
                    command.player.reply(messageFormatter_1.MessageFormatter.warning("Você não possui avatares!", "Use !loja avatar"));
                    return;
                }
                command.player.reply(messageFormatter_1.MessageFormatter.info("Avatares", "Use !avatar <emoji>"));
            ...
            if (!hasPurchase) {
                command.player.reply(messageFormatter_1.MessageFormatter.error("Você não possui este avatar!", "Compre na !loja avatar"));
                return;
            }
            ...
            command.player.reply(messageFormatter_1.MessageFormatter.success("Avatar equipado!", `${rarityInfo.emoji} ${hasPurchase.item.name}`));
        });
    }
            ...
            try {
                ...
                switch (effect.type) {
                    case "multiplier":
                        ...
                        player.reply(messageFormatter_1.MessageFormatter.success(`⚡ Boost ativado!`, `${effect.value}x por ${effect.durationHours}h`));
                        break;
                    case "coins":
                        yield this.addCoins(discordId, effect.value, `Item: ${item.name}`);
                        break;
                    case "gems":
                        const converted = (effect.value || 0) * LEGACY_GEM_TO_COINS_RATE;
                        yield this.addCoins(discordId, converted, `Item: ${item.name} (conversao legado)`);
                        player.reply(messageFormatter_1.MessageFormatter.economy(`🪙 Moedas creditadas!`, `+${formatCoins(converted)} moedas`));
                        break;
                    case "vip":
                        const vipExp = new Date(Date.now() + (effect.durationDays || 7) * 24 * 60 * 60 * 1000);
                        yield prisma_1.prisma.economy.update({ where: { discordId }, data: { vipExpires: vipExp } });
                        player.reply(messageFormatter_1.MessageFormatter.economy(`👑 VIP Ativado!`, `${effect.durationDays} dias`));
                        break;
                    case "avatar":
                        // Avatares não são aplicados automaticamente, precisam ser equipados
                        player.reply(messageFormatter_1.MessageFormatter.info(`Use !avatar ${effect.emoji} para equipar`));
                        break;
                    case "furafila":
                        // Furafila é consumido pelo FuraModule
                        player.reply(messageFormatter_1.MessageFormatter.success(`🚀 Furafila adquirido!`, `${effect.uses} usos`));
                        break;
                }
            }
            catch (_b) { }
    ...
    inventoryCommand(command) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (purchases.length === 0) {
                command.player.reply(messageFormatter_1.MessageFormatter.warning("Inventário vazio!", "Use !loja"));
                return;
            }
            command.player.reply(messageFormatter_1.MessageFormatter.info("Inventário", "Itens ativos"));
            for (const p of purchases) {
                const rarityInfo = ECONOMY_CONFIG.rarities[p.item.rarity] || ECONOMY_CONFIG.rarities.common;
                ...
                command.player.reply({
                    message: `${rarityInfo.emoji} ${p.item.name} (${(catInfo === null || catInfo === void 0 ? void 0 : catInfo.emoji) || "?"}) | ${st} ${usesText}`,
...
], EconomyModule.prototype, "onPlayerCorte", null);
__decorate([
    haxball_extended_room_1.CustomEvent
], EconomyModule.prototype, "onPlayerLevant", null);
__decorate([
    haxball_extended_room_1.CustomEvent
], EconomyModule.prototype, "onPlayerBlock", null);
__decorate([
    haxball_extended_room_1.Event
], EconomyModule.prototype, "onGameStart", null);
__decorate([
    haxball_extended_room_1.CustomEvent
], EconomyModule.prototype, "onTeamVictory", null);
__decorate([
    (0, haxball_extended_room_1.ModuleCommand)({
        aliases: ["coins", "saldo", "carteira", "wallet"],
        deleteMessage: true,
    })
], EconomyModule.prototype, "checkBalance", null);
__decorate([
    (0, haxball_extended_room_1.ModuleCommand)({
        aliases: ["odds"],
        deleteMessage: true,
    })
], EconomyModule.prototype, "oddsCommand", null);
__decorate([
    (0, haxball_extended_room_1.ModuleCommand)({
        aliases: ["daily", "diario"],
        deleteMessage: true,
    })
], EconomyModule.prototype, "dailyReward", null);
__decorate([
    (0, haxball_extended_room_1.ModuleCommand)({
        aliases: ["apostar", "bet"],
        deleteMessage: true,
    })
], EconomyModule.prototype, "betCommand", null);
__decorate([
    (0, haxball_extended_room_1.ModuleCommand)({
        aliases: ["tigrinho", "tiger", "slot", "slots"],
        deleteMessage: true,
    })
], EconomyModule.prototype, "tigrinhoCommand", null);
__decorate([
    (0, haxball_extended_room_1.ModuleCommand)({
        aliases: ["transferir", "pagar", "pay", "transfer"],
        deleteMessage: true,
    })
], EconomyModule.prototype, "transferCommand", null);
__decorate([
    (0, haxball_extended_room_1.ModuleCommand)({
        aliases: ["ranking", "ricos", "top", "leaderboard"],
        deleteMessage: true,
    })
], EconomyModule.prototype, "rankingCommand", null);
__decorate([
    (0, haxball_extended_room_1.ModuleCommand)({
        aliases: ["loja", "shop", "store"],
        deleteMessage: true,
    })
], EconomyModule.prototype, "shopCommand", null);
__decorate([
    (0, haxball_extended_room_1.ModuleCommand)({
        aliases: ["comprar", "buy"],
        deleteMessage: true,
    })
], EconomyModule.prototype, "buyCommand", null);
__decorate([
    (0, haxball_extended_room_1.ModuleCommand)({
        aliases: ["avatar", "av"],
        deleteMessage: true,
    })
], EconomyModule.prototype, "avatarCommand", null);
__decorate([
    (0, haxball_extended_room_1.ModuleCommand)({
        aliases: ["inventario", "inventory", "inv"],
        deleteMessage: true,
    })
], EconomyModule.prototype, "inventoryCommand", null);
