...
export class DiscordUtil {
    ...
    static async uploadFileToChannel(channelId: string, file: Buffer, name: string, description: string, embed?: EmbedBuilder) {
    ...
    static async sendEmbedInChannel(channel: GuildBasedChannel | string | null, embed: EmbedBuilder, description : string) {
        if(typeof channel === "string") channel = await DiscordUtil.getChannelById(channel);
        if(!channel) {
            console.log("log", `Channel not found.`);
            return;
